...
// ─── Engine ────────────────────────────────────────────────────────────────────
export class RansomwareGuard {
    id = EngineID.RANSOMWARE_GUARD;
    name = "RansomwareGuard · Triple-Extortion Defense";
    version = "1.9.0";
    /**
     * Calculate the blast radius of a ransomware attack on the given architecture.
     */
    calculateBlastRadius(architecture) {
        let totalDataGB = 0;
        let piiRecords = 0;
        ...
        for (const store of architecture.data_stores) {
            ...
            if (store.contains_pii)
                piiRecords += Math.round(store.size_gb * 10000); // Rough: ~10K records per GB
            if (store.contains_phi)
                piiRecords += Math.round(store.size_gb * 5000);
        }
    ...
    verifyBackupIntegrity(config) {
        ...
        if (!config.air_gapped)
            vulns.push('Backups are NOT air-gapped. Ransomware WILL encrypt them (97% attack rate on backups).');
        if (!config.immutable)
            vulns.push('Backups are mutable. Attackers can delete or encrypt backup files.');
        if (!config.tested_recovery)
            vulns.push('Backup recovery has NOT been tested. Recovery may fail under pressure.');
        if (config.retention_days < 30)
            vulns.push(`Retention is only ${config.retention_days} days. Attackers may wait before revealing the breach.`);
        if (config.strategy === 'local')
            vulns.push('Local-only backups can be encrypted alongside primary systems.');
        ...
        if (!config.air_gapped)
            recommendations.push('CRITICAL: Implement air-gapped backups immediately (offline/tape/isolated cloud vault).');
        if (!config.immutable)
            recommendations.push('Enable immutable/WORM storage for all backup targets.');
        if (!config.tested_recovery)
            recommendations.push('Schedule quarterly backup restoration drills.');
     ...
     */
    modelExtortionScenario(architecture) {
     ...
     */
    generateIsolationCommands(platform) {
        if (platform === 'windows') {
            return [
    ...
    async analyze(ctx) {
        ...
        if (ctx.networkSnapshot.outboundTrafficGbps > 15.0) {
            signals.push(createSignal(this.id, ThreatLevel.CRITICAL, `Massive outbound data flow detected (${ctx.networkSnapshot.outboundTrafficGbps} Gbps) — possible Stage-2 exfiltration`, {
                confidence: 0.95,
        ...
        if (highRiskUsers.length > 0) {
            signals.push(createSignal(this.id, ThreatLevel.HIGH, `Privileged account(s) showing extreme anomaly: ${highRiskUsers.map(u => u.userId).join(", ")}`, {
                confidence: 0.82,
     ...
     */
    solve(projectPath, architecture) {
