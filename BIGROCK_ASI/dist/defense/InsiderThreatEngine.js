...
export class InsiderThreatEngine {
    id = EngineID.INSIDER_THREAT;
    name = "InsiderThreatEngine · Behavioral UEBA & DLP";
    version = "2.1.0";
    profiles = new Map();
    /**
     * Build a behavioral baseline from git history and known developer patterns.
     */
    buildDeveloperProfile(projectPath, username) {
        ...
        try {
            ...
            if (fs.existsSync(gitLog)) {
                ...
                sensitiveFiles.push(...srcFiles.filter(f => /config|secret|auth|security|key|password|credential/i.test(f)).slice(0, 10));
            }
        }
        catch { /* skip */ }
        const profile = {
    ...
    analyzeBehavior(events) {
        ...
        for (const event of events) {
            if (!userEvents.has(event.user))
                userEvents.set(event.user, []);
        ...
        for (const [user, userEvts] of userEvents) {
            ...
            for (const pattern of EXFILTRATION_PATTERNS) {
                const matchingEvents = userEvts.filter(e => e.action === 'shell_command' &&
    ...
    async analyze(ctx) {
        ...
        if (offHours.length > 0) {
            signals.push(createSignal(this.id, ThreatLevel.HIGH, `${offHours.length} privileged user(s) active during off-hours with high anomaly score`, {
                confidence: 0.85,
        ...
        if (shadowAI) {
            signals.push(createSignal(this.id, ThreatLevel.CRITICAL, `Stage-2 Data Exfiltration detected: anomalous traffic to unauthorized AI endpoints (Shadow AI)`, {
                confidence: 0.92,
