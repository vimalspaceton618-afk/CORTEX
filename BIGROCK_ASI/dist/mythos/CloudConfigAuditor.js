...
const SHARED_RESPONSIBILITY_MODELS = {
    'AWS': {
        provider: [
...
// ─── Engine ────────────────────────────────────────────────────────────────────
export class CloudConfigAuditor {
    id = EngineID.CLOUD_CONFIG_AUDITOR;
    name = "CloudConfigAuditor · IaC & Middleware Security";
    version = "1.3.1";
    ...
    scanIaC(projectPath) {
        ...
        const scanFile = (filePath, rules, provider) => {
            try {
                ...
                if (resourceMatches)
                    resourcesAnalyzed += resourceMatches.length;
     ...
     */
    getSharedResponsibilityMap(provider, service) {
    ...
    async analyze(ctx) {
        ...
        const criticalConfigs = ctx.assetInventory.filter(a => a.type === "cloud-resource" && a.criticality === "mission_critical");
        if (criticalConfigs.length > 0) {
            signals.push(createSignal(this.id, ThreatLevel.CRITICAL, `${criticalConfigs.length} mission-critical cloud resource(s) found with insecure default configuration`, {
                confidence: 0.96,
        ...
        if (ldapMiddleware) {
            signals.push(createSignal(this.id, ThreatLevel.HIGH, `LDAP/SSO Middleware vulnerability detected (Oracle Cloud breach vector: 6M records at risk)`, {
                confidence: 0.88,
