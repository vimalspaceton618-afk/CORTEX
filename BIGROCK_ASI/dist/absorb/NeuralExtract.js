...
import { GGUFExtractor } from './GGUFExtractor.js';
export class NeuralExtract {
     ...
     */
    async extractFromGGUF(filepath) {
        ...
        if (emb_tensor_name) {
            ...
            if (emb_mb < 500) {
                ...
                if (emb_data) {
                    ...
                    const tokens = parsed.metadata['tokenizer.ggml.tokens'] || [];
    ...
    tokenize(text) {
        ...
        for (let wi = 0; wi < words.length; wi++) {
            ...
            if (found === undefined)
                found = this._token_lookup.get('▁' + word);
            if (found === undefined)
                found = this._token_lookup.get(word);
            if (found === undefined)
                found = this._token_lookup.get(word.toLowerCase());
            if (found === undefined)
                found = this._token_lookup.get('▁' + word.toLowerCase());
            if (found !== undefined) {
                result.push(found);
     ...
     */
    findSimilarTokens(text, top_k = 10) {
        if (!this.data?.embedding_index)
            return [];
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (!tok || tok.startsWith('<') || tok.length === 0 || norms[i] === 0 || isNaN(norms[i]))
                continue;
            const base = i * dim;
