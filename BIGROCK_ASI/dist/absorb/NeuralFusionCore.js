...
export class NeuralFusionCore {
    ...
    rebuildUnifiedIndex() {
        ...
        for (const knowledge of this.knowledge_bank.values()) {
            ...
            for (let i = 0; i < tokens.length; i++) {
                ...
                if (!token || token.startsWith('<') || token.length === 0 || norms[i] === 0)
                    continue;
                // Extract the vector for this token
                const vec = new Float32Array(dim);
                const base = i * dim;
                if (base + dim > vectors.length)
                    continue;
                for (let j = 0; j < dim; j++) {
     ...
     */
    crossModelSimilarity(text_a, text_b) {
        const per_model = [];
        for (const knowledge of this.knowledge_bank.values()) {
     ...
     */
    compareLayerDNA(filename_a, filename_b) {
        ...
        if (dna_a.length === 0 || dna_b.length === 0)
            return null;
        ...
        const architectural_diversity = Math.min(1.0, layer_deltas.reduce((s, v) => s + v, 0) / (layer_deltas.length || 1));
        ...
        if (are_complementary) {
            key_differences.unshift(`Models have COMPLEMENTARY architectures — together cover broader knowledge`);
        }
        else {
            key_differences.unshift(`Models have SIMILAR architectures — redundant but reliable consensus`);
        }
        return {
            model_a: filename_a,
            model_b: filename_b,
     ...
     */
    getUniversalVocab(sample_size = 100) {
