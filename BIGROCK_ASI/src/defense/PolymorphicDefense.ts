...
import {
  ...
  ResponseAction, type EngineStatus, createSignal, type UserActivityRecord,
} from "../types.js";

/**
 * BIGROCK_v1 — Engine #1: PolymorphicDefense
 * ─────────────────────────────────────────────────────────────────────────
 * ADDRESSES: AI-Powered Cyber Attacks — Severity 9.2/10 | $10.5T annual cost
 * UNSOLVED GAP: No mature defensive AI can match the adaptability of offensive polymorphic AI agents.
 *
 * This engine implements:
 *   1. BEHAVIORAL BASELINE ENGINE — Builds a rolling semantic fingerprint of system behavior
 *   2. SEMANTIC DRIFT DETECTOR — Flags deviation from baseline (polymorphic threat indicator)
 *   3. SHADOW AI SCANNER — Detects unsanctioned LLM API usage with corporate context
 *   4. AIBOM GENERATOR — AI Bill of Materials for governance compliance
 *   5. PROMPT INJECTION SIMULATOR — Adversarial simulation against own agent outputs
 */
...
export interface DriftAlert {
    type: 'file_mutation' | 'new_api_pattern' | 'env_exposure' | 'dependency_change' | 'loc_spike';
...
export interface ShadowAIReport {
    detected: boolean;
    instances: ShadowAIInstance[];
    risk_level: 'none' | 'low' | 'medium' | 'high' | 'critical';
    recommendations: string[];
}

export interface ShadowAIInstance {
    file: string;
    line: number;
    pattern: string;
    risk: string;
...
export interface AIBOM {
    generated_at: number;
    version: string;
    ai_models: AIBOMModel[];
    ai_agents: AIBOMAgent[];
    ai_apis: AIBOMAPIUsage[];
    data_sources: string[];
    total_risk_score: number;
    governance_status: 'compliant' | 'non_compliant' | 'partial';
    signature: string;
}
...
export interface AIBOMAPIUsage {
    endpoint: string;
    file: string;
    line: number;
    auth_type: string;
...
export interface InjectionVulnerability {
    vector: string;
    payload: string;
    result: 'blocked' | 'bypassed' | 'partial_bypass';
    severity: 'low' | 'medium' | 'high' | 'critical';
...
export class PolymorphicDefense implements CyberEngine {
    readonly id      = EngineID.POLYMORPHIC_DEFENSE;
    readonly name    = "PolymorphicDefense · AI Attack Countermeasure";
    readonly version = "3.1.0";

    private baseline: BehavioralProfile | null = null;
    private mutationWindows = new Map<string, any>();

    /**
     * Build a behavioral baseline of the project — file hashes, API patterns,
     * environment variable exposure, and LOC metrics.
     */
    public buildBehavioralBaseline(projectPath: string): BehavioralProfile {
        const fileHashes = new Map<string, string>();
        const apiPatterns = new Set<string>();
        const envExposure = new Set<string>();
        let totalFiles = 0;
        let totalLoc = 0;
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        ...
                        if (envMatches) envMatches.forEach((m: string) => envExposure.add(m));
     ...
     */
    public detectSemanticDrift(current: BehavioralProfile, baseline?: BehavioralProfile): DriftAlert[] {
        const base = baseline || this.baseline;
        if (!base) return [{ type: 'file_mutation', severity: 'high', description: 'No baseline available. Run buildBehavioralBaseline() first.' }];
     ...
     */
    public scanForShadowAI(projectPath: string): ShadowAIReport {
        const instances: ShadowAIInstance[] = [];
        ...
        const riskLevel: ShadowAIReport['risk_level'] = instances.length === 0 ? 'none'
     ...
     */
    public generateAIBOM(projectPath: string): AIBOM {
        const models: AIBOMModel[] = [];
        const apis: AIBOMAPIUsage[] = [];
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        ...
                        for (const pat of dataPatterns) {
                            ...
                            if (matches) matches.forEach((m: string) => dataSources.add(m));
                        }
                    } catch { /* skip */ }
                }
            }
        ...
        return {
            ...
            governance_status: governance as any,
            signature,
        };
    }
     ...
     */
    public simulatePromptInjection(
        securityScanFn: (input: string) => { clean: boolean; reason?: string }
    ): InjectionVulnerability[] {
        const results: InjectionVulnerability[] = [];
    ...
    async analyze(ctx: AnalysisContext): Promise<ThreatSignal[]> {
        ...
        if (anomalyScore > 30) {
            ...
            signals.push(createSignal(
                this.id,
                mutationRate > 300 ? ThreatLevel.CRITICAL : ThreatLevel.HIGH,
                `Polymorphic payload mutating at ${mutationRate} variants/hr — signature-based detection bypassed`,
                {
                    confidence: 0.85,
        ...
        const violators = ctx.userActivity.filter(u => 
            u.actions.some(a => ["openai", "anthropic", "gemini"].some(api => a.includes(api)))
        ).map(u => u.userId);

        if (violators.length > 0) {
            signals.push(createSignal(
                this.id,
                violators.length > 5 ? ThreatLevel.HIGH : ThreatLevel.MODERATE,
                `Shadow AI data leak: ${violators.length} user(s) uploading sensitive data to unsanctioned AI models`,
                {
                    confidence: 0.92,
    ...
    private classifyDataExposure(lines: string[], lineIndex: number): string {
        // Check surrounding lines for sensitive data patterns
        const context = lines.slice(Math.max(0, lineIndex - 5), lineIndex + 5).join('\n');
        if (/password|secret|api_key|token|bearer|authorization/i.test(context)) return 'AUTH';
        if (/email|phone|ssn|name|address|dob|date_of_birth/i.test(context)) return 'PII';
        if (/\.env|process\.env|config/i.test(context)) return 'CONFIG';
        if (/source_code|import|require/i.test(context)) return 'CODE';
        return 'GENERAL';
    }
     ...
     */
    public solve(projectPath: string, shadowResult?: ShadowAIReport): import('../mythos/PQCAuditor.js').RemediationScript {
