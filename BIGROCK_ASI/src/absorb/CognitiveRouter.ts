...
export interface QueryAnalysis {
    ...
    /** All domains detected (multi-domain queries) */
    all_domains: CognitiveDomain[];
    ...
    /** Does this require high-confidence answer? */
...
export interface ComplexityBreakdown {
    length_score: number;       // 0–15: long queries = higher complexity
    multi_domain_score: number; // 0–20: multiple domains = higher complexity
    logical_ops_score: number;  // 0–15: AND/OR/NOT/IMPLIES = logical complexity
    uncertainty_score: number;  // 0–10: "might", "could", "would" = uncertain
    code_presence_score: number;// 0–15: code blocks, function refs = code task
    math_symbols_score: number; // 0–10: equations, integrals, derivatives
    depth_keywords_score: number;// 0–10: "explain", "compare", "analyze", "design"
    negation_score: number;     // 0–5: "not", "never", "cannot" = careful reasoning
}
...
export interface RoutingDecision {
    ...
    /** Secondary models (for blend/consensus/chain) */
    secondary_models: ModelEntry[];
    /** All models in priority order */
    all_models: ModelEntry[];
    /** Confidence in this routing decision */
    routing_confidence: number;
    /** Human-readable routing trace */
    trace: string[];
}
...
export class CognitiveRouter {
     ...
     */
    public analyseQuery(query: string): QueryAnalysis {
        const lower = query.toLowerCase();
        const words = lower.split(/\s+/);

        // ── Domain Detection ──────────────────────────────────────────────────
        const domain_hits: Map<CognitiveDomain, number> = new Map();
        ...
        for (const [domain, keywords] of Object.entries(DOMAIN_KEYWORDS) as [CognitiveDomain, string[]][]) {
            ...
            if (hits > 0) domain_hits.set(domain, hits);
        }
        ...
        const primary_domain: CognitiveDomain =
            sorted_domains.length > 0 ? sorted_domains[0] : 'unknown';

        const all_domains: CognitiveDomain[] =
            sorted_domains.length > 0 ? sorted_domains : ['unknown'];
        ...
        const requires_high_confidence =
            HIGH_CONFIDENCE_MARKERS.some(m => lower.includes(m)) ||
            primary_domain === 'cybersecurity' ||
            primary_domain === 'mathematics';

        // ── Strategy Selection ────────────────────────────────────────────────
        const { strategy, reason } = this.selectStrategy(
            complexity_score,
            is_multi_domain,
    ...
    private computeComplexity(lower: string, words: string[], domains: CognitiveDomain[]): ComplexityBreakdown {
        ...
        const logical_ops_score = Math.min(15,
            logical_ops.filter(op => lower.includes(op)).length * 3
        );

        // Uncertainty markers (0–10)
        const uncertainty_words = ['might', 'could', 'would', 'perhaps', 'maybe', 'likely', 'possibly'];
        const uncertainty_score = Math.min(10,
            uncertainty_words.filter(w => words.includes(w)).length * 3
        );

        // Code presence (0–15)
        const code_markers = ['```', '()', '=>', '==', '!==', '===', 'function', 'class ', 'def ', 'import ', 'return'];
        const code_presence_score = Math.min(15,
            code_markers.filter(m => lower.includes(m)).length * 4
        );

        // Math symbols (0–10)
        const math_markers = ['=', '+', '-', '×', '÷', '^', '∫', '∑', 'π', 'σ', 'dx', 'dy'];
        const math_symbols_score = Math.min(10,
            math_markers.filter(m => lower.includes(m)).length * 2
        );

        // Depth keywords (0–10)
        const depth_keywords_score = Math.min(10,
            DEPTH_KEYWORDS.filter(kw => lower.includes(kw)).length * 3
        );

        // Negation (0–5)
        const negation_words = ['not', 'never', 'cannot', "can't", "doesn't", 'no ', 'none', 'without'];
        const negation_score = Math.min(5,
            negation_words.filter(w => lower.includes(w)).length * 1.5
        );
    ...
    ): { strategy: RoutingStrategy; reason: string } {
    ...
    public route(
        ...
        override_strategy?: RoutingStrategy,
    ): RoutingDecision {
        ...
        let secondary_models: ModelEntry[] = [];
        ...
        switch (strategy) {
            ...
            case 'consensus_panel': {
                ...
                const scores = [primary_model, ...secondary_models].map(m =>
                    m.profile.domain_scores.find(d => d.domain === domain)?.score ?? 0
                );
     ...
     */
    private rankModelsForDomain(models: ModelEntry[], domain: string): ModelEntry[] {
     ...
     */
    public quickDomain(query: string): CognitiveDomain {
        const lower = query.toLowerCase();
        let best: CognitiveDomain = 'unknown';
        let best_hits = 0;
     ...
     */
    public detectAllDomains(query: string): { domain: CognitiveDomain; confidence: number }[] {
        const lower = query.toLowerCase();
        const results: { domain: CognitiveDomain; confidence: number }[] = [];
