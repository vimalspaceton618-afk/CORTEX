...
export interface ProbeQuestion {
    ...
    /** Patterns that SHOULD appear in a good answer (case-insensitive) */
    expected_markers: (string | RegExp)[];
    /** Patterns that indicate a BAD answer */
    negative_markers?: (string | RegExp)[];
    /** Weight of this probe (1.0 = standard) */
    weight: number;
}
...
export interface DomainScore {
    ...
    probes_run: number;
    probes_passed: number;  // score >= 0.5
    probe_results: ProbeResult[];
}

// ─── Probe Bank ────────────────────────────────────────────────────────────────

const PROBE_BANK: ProbeQuestion[] = [
...
export class DomainProber {
    ...
    private probes: ProbeQuestion[] = PROBE_BANK;

    /**
     * Get all probes for a specific domain.
     */
    public getProbesForDomain(domain: string): ProbeQuestion[] {
     ...
     */
    public scoreResponse(probe: ProbeQuestion, response: string): ProbeResult {
        const lower = response.toLowerCase();
        const markers_hit: string[] = [];
        const markers_missed: string[] = [];
        ...
        if (probe.negative_markers) {
            for (const marker of probe.negative_markers) {
                const pattern = typeof marker === 'string'
                    ? new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i')
                    : marker;
                if (pattern.test(response)) negative_hits++;
            }
        }
     ...
     */
    public aggregateDomainScores(results: ProbeResult[]): DomainScore[] {
    ...
    /** Total probe count */
    public getProbeCount(): number { return this.probes.length; }
}
