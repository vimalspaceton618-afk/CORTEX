...
export class SLMEngine {
    ...
    constructor(options: SLMOptions = {}) {
        ...
        if (fs.existsSync(this.modelPath)) {
            ...
            if (process.env.DEBUG === 'true') console.log(`[SLMEngine]: No .gguf model found at ${this.modelPath}. SLM fallback disabled.`);
    ...
    private async initModel() {
        ...
        try {
            if (isDebug) console.log(`[SLMEngine]: Loading native GGUF model from ${this.modelPath} (CPU only)...`);
            ...
            if (isDebug) console.log(`[SLMEngine]: Native SLM (Phi-3) successfully loaded into memory.`);
        } catch (e: any) {
            if (isDebug) console.error(`[SLMEngine Error]: Failed to load GGUF model: ${e.message}`);
            this.is_available = false;
        }
    }
