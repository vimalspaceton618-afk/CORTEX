...
async function runTest(name: string, description: string, runner: (core: CognitionCore) => Promise<{ passed: boolean, details: string }>): Promise<void> {
...
async function main() {
    ...
    await runTest(
        "Test 1: The Grand Synthesis (DAG Decomposition)",
        "Splinter a multi-domain query across Physics, Math, and Logic.",
        async (core) => {
    ...
    await runTest(
        ...
        "Force a predictive fallback response and monitor the SelfReflectionEngine.",
        async (core) => {
    ...
    await runTest(
        "Test 3: Episodic Time-Travel",
        "Perform conceptual semantic vector search using 3072D embeddings.",
        async (core) => {
    ...
    await runTest(
        "Test 4: Continuous Learning Load",
        "Hammer the ContinuousLearningCore and monitor ODE-based entropy shifts.",
        async (core) => {
    ...
    await runTest(
        "Test 5: Zero-Trust Cortex Breach (Data Exfiltration)",
        "Attempt a severe prompt injection with simulated privilege escalation.",
        async (core) => {
    ...
    await runTest(
        "Test 6: Cognitive Paradox DOS (Contradiction)",
        "Force the formal logic prover into a paradox to verify sandbox bounds.",
        async (core) => {
    ...
    await runTest(
        "Test 7: Mythos Autonomous Threat Hunting",
        "Scan an architectural context for Myth Gaps using the neuro-symbolic engine.",
        async (core) => {
