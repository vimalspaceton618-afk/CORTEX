...
/**
 * BIGROCK_v1 — Engine #7: IoTVulnScanner
 * ─────────────────────────────────────────────────────────────────────────
 * ADDRESSES: IoT / IoMT Vulnerabilities — Severity 7.9/10 | 18.5B devices | $10M healthcare breach cost
 * UNSOLVED GAP: Manufacturers have no economic incentive to invest in security. Legacy device volume is massive.
 *
 * This engine implements:
 *   1. IOT ATTACK SURFACE MAPPER — Maps device categories and vulnerability profiles
 *   2. IOMT PATIENT SAFETY RISK SCORER — Flags medical devices with safety impact
 *   3. EU CYBER RESILIENCE ACT COMPLIANCE CHECKER — Validates against EU CRA
 *   4. SEGMENTATION RECOMMENDATION ENGINE — Firewall rules and VLAN isolation
 */
...
export interface IoTDevice {
    name: string;
    category: 'industrial' | 'medical' | 'consumer' | 'building' | 'automotive' | 'networking' | 'wearable';
    manufacturer: string;
    firmware_version: string;
    last_update: string;
    protocol: string; // MQTT, CoAP, HTTP, BLE, Zigbee, etc.
    has_default_credentials: boolean;
    internet_facing: boolean;
    data_collected: string[];
    criticality: 'life_safety' | 'mission_critical' | 'operational' | 'convenience';
}

export interface IoTAttackSurfaceReport {
    total_devices: number;
    devices_by_category: Record<string, number>;
    critical_vulnerabilities: IoTVulnerability[];
    risk_score: number;
    risk_grade: 'A' | 'B' | 'C' | 'D' | 'F';
    recommendations: string[];
}

export interface IoTVulnerability {
    device: string;
...
export interface IoMTSafetyReport {
    total_medical_devices: number;
    life_safety_devices: number;
    safety_score: number; // 0-100
    critical_findings: IoMTFinding[];
    regulatory_compliance: { fda_510k: boolean; hipaa: boolean; iec_62443: boolean };
    estimated_breach_cost_usd: number;
}

export interface IoMTFinding {
    device: string;
    finding: string;
...
export interface CRAComplianceReport {
    compliant: boolean;
    checks: CRACheck[];
    non_compliant_devices: string[];
    penalties_at_risk: string;
}

export interface CRACheck {
    requirement: string;
    article: string;
...
const IOT_VULN_PATTERNS: Record<string, { vulns: { name: string; severity: IoTVulnerability['severity']; cwe: string; vector: string; fix: string }[] }> = {
    default_creds: {
        vulns: [
...
export class IoTVulnScanner implements CyberEngine {
    readonly id      = EngineID.IOT_VULN_SCANNER;
    readonly name    = "IoTVulnScanner · Air-Gapped IoT/IoMT Auditor";
    readonly version = "1.7.0";

    /**
     * Map the IoT attack surface from a device inventory.
     */
    public mapAttackSurface(devices: IoTDevice[]): IoTAttackSurfaceReport {
        const vulns: IoTVulnerability[] = [];
     ...
     */
    public checkCRACompliance(devices: IoTDevice[]): CRAComplianceReport {
     ...
     */
    public generateSegmentationPlan(devices: IoTDevice[]): SegmentationPlan {
    ...
    async analyze(ctx: AnalysisContext): Promise<ThreatSignal[]> {
        ...
        const medicalDevices = ctx.assetInventory.filter(a => (a.type as any) === "medical-device" && (a.criticality as any) === "life_safety");
        if (medicalDevices.length > 0) {
            ...
            if (exposed.length > 0) {
                 signals.push(createSignal(
                    ...
                    `${exposed.length} life-safety IoMT device(s) directly exposed to internet (CRA violation)`,
                    {
                        confidence: 0.99,
        ...
        if (scadaTraffic) {
             signals.push(createSignal(
                this.id,
                ThreatLevel.HIGH,
                `Anomalous traffic detected in ICS/SCADA segment — potential lateral movement attempt`,
                {
                    confidence: 0.82,
     ...
     */
    public solve(projectPath: string, devices?: IoTDevice[]): import('../mythos/PQCAuditor.js').RemediationScript {
