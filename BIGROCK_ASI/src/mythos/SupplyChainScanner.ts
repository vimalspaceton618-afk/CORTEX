...
/**
 * BIGROCK_v1 — Engine #5: SupplyChainScanner
 * ─────────────────────────────────────────────────────────────────────────
 * ADDRESSES: Supply Chain Attack Complexity — Severity 8.3/10 | 700% growth in 5 years
 * UNSOLVED GAP: No real-time vendor posture visibility. Static questionnaires are useless.
 *
 * This engine implements:
 *   1. FULL DEPENDENCY GRAPH BUILDER — Resolves entire transitive dependency tree
 *   2. AIBOM GENERATOR — Signed AI Bill of Materials
 *   3. OAUTH TOKEN RISK ANALYZER — Detects overly-broad token scopes
 *   4. THIRD-PARTY INTEGRATION MAPPER — Vendor risk graph
 */
...
export interface DependencyGraph {
    ...
    risk_summary: {
        ...
        no_integrity_hash: number;
        deeply_nested: number; // depth > 5
    };
}

export interface SupplyChainAIBOM {
    generated_at: number;
    project_name: string;
    version: string;
    dependencies: AIBOMDependency[];
    local_models: AIBOMLocalModel[];
...
export interface AIBOMLocalModel {
    filename: string;
    format: string;
    size_bytes: number;
    hash: string;
...
export interface OAuthRiskReport {
    total_tokens_found: number;
    high_risk_tokens: OAuthToken[];
...
export interface VendorRiskGraph {
...
export interface VendorNode {
    name: string;
    type: 'saas' | 'api' | 'cdn' | 'payment' | 'auth' | 'monitoring' | 'storage';
    data_access: string[];
...
const VENDOR_PATTERNS: { regex: RegExp; name: string; type: VendorNode['type']; data_access: string[] }[] = [
...
export class SupplyChainScanner implements CyberEngine {
    readonly id      = EngineID.SUPPLY_CHAIN_SCANNER;
    readonly name    = "SupplyChainScanner · Continuous Inventory & SBOM";
    readonly version = "3.4.0";

    /**
     * Build the full transitive dependency graph from lockfiles.
     */
    public buildDependencyGraph(projectPath: string): DependencyGraph {
     ...
     */
    public generateAIBOM(projectPath: string): SupplyChainAIBOM {
        const graph = this.buildDependencyGraph(projectPath);

        const dependencies: AIBOMDependency[] = graph.nodes.map(n => ({
            name: n.name,
            version: n.version,
            license: n.license,
        ...
        const localModels: AIBOMLocalModel[] = [];
     ...
     */
    public analyzeOAuthTokens(projectPath: string): OAuthRiskReport {
        ...
        const riskLevel: OAuthRiskReport['risk_level'] = highRisk.length > 3 ? 'critical'
     ...
     */
    public mapThirdPartyIntegrations(projectPath: string): VendorRiskGraph {
        const vendorMap = new Map<string, VendorNode>();
        ...
        const overall: VendorRiskGraph['overall_risk'] = criticalPaths.length > 3 ? 'critical'
    ...
    async analyze(ctx: AnalysisContext): Promise<ThreatSignal[]> {
        ...
        if (maliciousPackages.length > 0) {
            signals.push(createSignal(
                ...
                `Malicious dependency pattern detected: ${maliciousPackages.map(p => p.ioc).join(", ")} — Possible typo-squatting or brand-jacking`,
                {
                    confidence: 0.94,
        ...
        if (broadTokens.length > 0) {
            signals.push(createSignal(
                this.id,
                ThreatLevel.HIGH,
                `${broadTokens.length} OAuth token(s) found with global scope (e.g., Salesforce/Drift vector)`,
                {
                    confidence: 0.89,
     ...
     */
    public solve(projectPath: string, oauthResult?: OAuthRiskReport): import('../mythos/PQCAuditor.js').RemediationScript {
