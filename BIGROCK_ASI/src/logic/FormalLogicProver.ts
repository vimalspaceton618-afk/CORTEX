...
export type LogicValue = true | false;

export interface TruthTableRow {
    variables: Record<string, LogicValue>;
    result: LogicValue;
}
...
export class FormalLogicProver {
     ...
     */
    private evaluate(normalized: string, bindings: Record<string, LogicValue>): LogicValue {
        let expr = normalized;
        ...
        while (expr !== 'true' && expr !== 'false' && safety < 200) {
            ...
            // Step 3: AND
            expr = expr.replace(/(true|false)\s*&&\s*(true|false)/, (_, a, b) =>
            ...
            // Step 6: IMPLICATION
            expr = expr.replace(/(true|false)\s+IMPL\s+(true|false)/, (_, a, b) =>
                (a !== 'true' || b === 'true') ? 'true' : 'false'
            );

            // Step 7: BICONDITIONAL
            expr = expr.replace(/(true|false)\s+BICON\s+(true|false)/, (_, a, b) =>
                (a === b) ? 'true' : 'false'
            );
     ...
     */
    public prove(expression: string): LogicResult {
        ...
        for (let i = 0; i < totalRows; i++) {
            const bindings: Record<string, LogicValue> = {};
     ...
     */
    public areEquivalent(expr1: string, expr2: string): { equivalent: boolean; differences: number[] } {
