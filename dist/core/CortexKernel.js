...
let CognitionCoreClass = null;
let AutonomousReasoningLoopClass = null;
...
import { BeastMode } from './BeastMode.js';
// ─── CORTEX KERNEL ──────────────────────────────────────────────────────────
export class CortexKernel {
    static instance = null;
    core = null; // CognitionCore instance
    arl = null; // AutonomousReasoningLoop instance
    beast;
    booted = false;
    boot_time = 0;
    bigrock_loaded = false;
    constructor() {
        this.beast = new BeastMode((config) => {
    ...
    engageBeastMode() {
        ...
        if (status.config.auto_absorb_on_boot && this.core) {
            ...
            if (!absorber.hasAbsorbedModels()) {
                absorber.absorbAll(false).catch(() => { });
            }
        }
        return status;
    }
    ...
    getNativeBrainInfo() {
        ...
        return {
            ...
            champions: Object.fromEntries(Object.entries(status.hive_status.domain_coverage).map(([k, v]) => [k, v.champion])),
