...
function formatProviderFailure(candidate, error) {
...
function resolveLocalModelForAgent(agentName) {
    if (agentName === 'PlanAgent' || agentName === 'ExploreAgent') {
...
function getBrainCandidates(agentName) {
    ...
    const providerMap = { local, cloud, native };
    const rawRoutingMap = process.env.BRAIN_ROUTING_MAP?.trim();
    if (rawRoutingMap) {
        try {
            const parsed = JSON.parse(rawRoutingMap);
...
// ──────────────────────────────────────────────────────────────
function getCloudFallbackCandidates(failedModel) {
...
// ──────────────────────────────────────────────────────────────
async function* attemptCandidate(candidate, messages, toolsConfig, agentName) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES_PER_CANDIDATE; attempt++) {
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Cloud request timed out after ${CLOUD_TIMEOUT_MS / 1000}s`)), CLOUD_TIMEOUT_MS));
...
export class BaseAgent {
    ...
    // ──────────────────────────────────────────────────────────
    async *resolveBrain(messages, toolsConfig) {
        const candidates = getBrainCandidates(this.name);
        const providerErrors = [];
        let failedCloudModel = null;
    ...
    async *run(input, requestConfirmation) {
        ...
        while (keepRunning) {
            ...
            try {
                ...
                const brainResolver = this.resolveBrain(this.history, toolsConfig);
                let resolved = null;
                while (true) {
                    ...
                    for (const toolCall of toolCallsArray) {
                        ...
                        if (tool) {
                            try {
                                ...
                                if (executionResult && typeof executionResult[Symbol.asyncIterator] === 'function') {
