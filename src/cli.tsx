...
async function runHeadless(prompt: string, asJson: boolean, autoApprove: boolean, beastMode: boolean) {
  ...
  if (normalized === '/cyberscan') {
    ...
    if (asJson) process.stdout.write(JSON.stringify({ prompt, result }, null, 2) + '\n');
  ...
  if (normalized === '/cyberheal') {
    ...
    if (asJson) process.stdout.write(JSON.stringify({ prompt, scripts }, null, 2) + '\n');
    else process.stdout.write(out + '\n');
    return;
  }
  ...
  if (normalized === '/brain eat') {
    if (!CortexKernel.isBooted()) { process.stdout.write('⚠ Kernel not booted.\n'); return; }
    try {
      ...
      if (asJson) process.stdout.write(JSON.stringify({ prompt, status: bStat }, null, 2) + '\n');
      else process.stdout.write(out + '\n');
    } catch (e: any) {
  ...
  if (normalized === '/brain') {
    ...
    if (hasNative && info) {
      ...
      if (asJson) process.stdout.write(JSON.stringify({ prompt, info }, null, 2) + '\n');
      else process.stdout.write(out + '\n');
    } else {
      const out = '🧠 Active Brain: CLOUD AI\nRun /brain eat to activate local sovereign AI.';
      if (asJson) process.stdout.write(JSON.stringify({ prompt, info: 'cloud' }, null, 2) + '\n');
      else process.stdout.write(out + '\n');
    }
    return;
  }
  ...
  if (normalized === '/arl' || normalized === '/arl status') {
    ...
    if (asJson) process.stdout.write(JSON.stringify({ prompt, metrics: m, running }, null, 2) + '\n');
    else process.stdout.write(out + '\n');
    return;
  }
  ...
  if (normalized.startsWith('/mythos ')) {
    ...
    if (!gaps.length) {
      ...
      if (asJson) process.stdout.write(JSON.stringify({ prompt, gaps }, null, 2) + '\n');
      else process.stdout.write(out + '\n');
    }
    return;
  }
  ...
  // ─── /demo ───────────────────────────────────────────────────
  if (normalized === '/demo') {
    if (!CortexKernel.isBooted()) { process.stdout.write('⚠ Kernel not booted. Run npm run build in BIGROCK_ASI/ first.\n'); return; }
